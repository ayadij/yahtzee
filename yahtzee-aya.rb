...
module Yahtzee
  ...
  class Roll
    ...
    # Count occurrences of a set of dots.
    def count *dots 
      @dice.inject(0) do |total, die|
        if dots.include?(die) then total + 1 else total end
      end
    end
  
    # Add all occurrences of a set of dots, or all the dice.
    def sum *dots 
      if dots.size == 0
        @dice.inject(0) { |total, die| total + die }
      else
        @dice.inject(0) do |total, die|
          if dots.include?(die) then total + die else total end
        end
      end
    end
    ...
    def matches_letters? letters 
      ...
      dots = @dice.uniq
      counts.each do |c|
        return false unless match = dots.find { |p| count(p) >= c }
        dots.delete(match)
      end
...
if __FILE__ == $0
  ...
  13.times do
    ...
    while rolls > 0
      puts "\nYOUR ROLL: #{roll}"
      print "  Enter the numbers would you like to reroll or...\n  (c)heck score\n  (s)corecard\n  (q)uit\n"
      ...
      when /^q/i
        ...
        begin
          dots = choice.gsub(/\s+/, "").split(//).map do |n|
            Integer(n)
          end
          roll.reroll(*dots)
          rolls -= 1
        rescue
          puts "Error: invalid reroll."
        end
      end
    end
    ...
    loop do
      ...
      print "\nScore:\n#{score}\n" +
            "Enter the scorecard category number of where you would like to count your roll\n #{roll} "
      begin
